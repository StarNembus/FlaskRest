...
def update_tutorial(tutorial_id):
    item = next((x for x in tutorials if x['id'] == tutorial_id), None) # генератор вернет элемент с соотв id или None
    params = request.json
    if not item:
...
def delete_tutorial(tutorial_id):
    idx, _ = next((x for x in enumerate(tutorials)
                   if x[1]['id'] == tutorial_id), (None, None)) # получение индекса элемента
